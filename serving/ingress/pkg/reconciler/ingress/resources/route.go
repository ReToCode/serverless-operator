...
func makeRoute(ci *networkingv1alpha1.Ingress, host string, rule networkingv1alpha1.IngressRule) (*routev1.Route, error) {
	...
	// * the system-internal-tls is enabled.
	if _, ok := annotations[EnablePassthroughRouteAnnotation]; ok ||
		...
		isSystemInternalTLSEnabled(rule) {
...
// isSystemInternalTLSEnabled determines whether system-internal-tls is enabled or not.
// In general, we can determine it by the value system-internal-tls in config-network, however the serverless ingress does not
// watch the ConfigMap. Therefore, we determine it by ServiceHTTPSPort(443) port for the backend in Kingress.
func isSystemInternalTLSEnabled(rule networkingv1alpha1.IngressRule) bool {
	if rule.HTTP == nil {
		return false
	}
